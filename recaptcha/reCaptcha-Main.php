...
function setup_recaptcha($verify_id)
{
	...
	if (!empty($settings['recaptcha_public_key']) && !empty($settings['recaptcha_private_key']))
	{
...
function validate_recaptcha(&$verify_id, &$verification_errors)
{
	...
	$resp = recaptcha_check_answer($settings['recaptcha_private_key'], $_SERVER['REMOTE_ADDR'], $_POST['recaptcha_challenge_field'], $_POST['recaptcha_response_field']);
	if (!$resp->is_valid)
