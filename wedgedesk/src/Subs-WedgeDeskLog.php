...
function shd_load_action_log_entries($start = 0, $items_per_page = 10, $sort = 'la.log_time', $order = 'DESC', $clause = '')
{
	...
	foreach ($actions as $k => $action)
	{
		...
		if ($action['action'] == 'notify' && isset($action['extra']['emails']))
		{
			...
			{
				$this_content = '<br><a href="<URL>?action=helpdesk;sa=emaillog;log=' . $action['id'] . ';template=' . $email_type . '" onclick="return reqWin(this.href);">' . $txt['template_log_notify_' . $email_type] . '</a> - ';
