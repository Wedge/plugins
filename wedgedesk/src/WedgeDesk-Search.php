...
function shd_search()
{
	...
	loadPluginTemplate('Arantor:WedgeDesk', 'tpl/WedgeDesk-Search');
...
function shd_search2()
{
	...
	// Lastly, page number. We're doing something different to Wedge's normal style here. Long and complicated, but there you go.
	if (isset($_POST['page']))
	...
	if (empty($context['search_terms']))
	{
		...
		while ($row = wesql::fetch_assoc($query))
		{
			...
			$row['display_id'] = str_pad($row['id_ticket'], $settings['shd_zerofill'], '0', STR_PAD_LEFT);
			...
			$row['dept_link'] = !$context['shd_multi_dept'] ? '' : '[<a href="<URL>?action=helpdesk;sa=main;dept=' . $row['id_dept'] . '">' . $row['dept_name'] . '</a>] ';
	...
	if (!empty($_POST[$field . '_name']))
	{
		...
		foreach ($namedlist as $index => $name)
			if (strlen(trim($name)) > 0)
				$namedlist[$index] = westr::htmlspecialchars(westr::strtolower(trim($name)));
			else
