 ...
 * WedgeDesk
 *
 * This file handles the sending of email notifications on ticket changes.
 *
 * @package wedgedesk
...
function shd_notifications_notify_newticket(&$msgOptions, &$ticketOptions, &$posterOptions)
{
	...
	// Enabled?
	if (empty($settings['shd_notify_new_ticket']))
		return;
	...
	$members = shd_get_visible_list($ticketOptions['dept'], $ticketOptions['private'], false, empty($settings['shd_admins_not_assignable']), false);
	if (empty($members))
		return;
	...
	$notify_data = array(
		'members' => $members,
		'ticketlink' => '<URL>?action=helpdesk;sa=ticket;ticket=' . $ticketOptions['id'],
...
function shd_notifications_notify_newreply(&$msgOptions, &$ticketOptions, &$posterOptions)
{
	...
	$staff = shd_get_visible_list($ticketOptions['dept'], $ticketinfo['private'], $ticketinfo['starter_id'], empty($settings['shd_admins_not_assignable']), false);

	// Might as well kick this off here.
	$notify_data = array(
		'members' => array(),
		'ticketlink' => '<URL>?action=helpdesk;sa=ticket;ticket=' . $ticketOptions['id'] . '.msg' . $msgOptions['id'] . '#msg' . $msgOptions['id'],
	...
	if (!empty($ticketinfo['starter_id']) && !empty($settings['shd_notify_new_reply_own']) && $posterOptions['id'] != $ticketinfo['starter_id'] && in_array($ticketinfo['starter_id'], $staff))
	...
	if (!empty($settings['shd_notify_new_reply_assigned']))
...
function shd_notifications_notify_assign(&$ticket, &$assignment)
{
	...
	$notify_data = array(
		'members' => array(),
		'ticketlink' => '<URL>?action=helpdesk;sa=ticket;ticket=' . $ticket . '.0',
		'subject' => $ticketinfo['subject'],
		'ticket' => $ticket,
	);
	...
	if (!empty($ticketinfo['starter_id']) && !empty($settings['shd_notify_assign_own']))
	{
	...
	if (!empty($assignment) && !empty($settings['shd_notify_assign_me']))
	{
...
function shd_notify_users($notify_data)
{
	...
	$replacements = array(
		'{ticket_id}' => str_pad($notify_data['ticket'], $settings['shd_zerofill'], '0', STR_PAD_LEFT),
	...
	$withbody = isset($notify_data['body']) && !empty($settings['shd_notify_with_body']) ? 'bodyfull' : 'body';
	...
	{
		loadPluginLanguage('Arantor:WedgeDesk', 'lang/WedgeDeskNotifications', $this_lang, true);
		...
		foreach ($lang_members as $member)
		{
			...
			if (!empty($settings['shd_notify_email']))
				$settings['mail_from'] = $settings['shd_notify_email'];
	...
	if (empty($settings['shd_disable_action_log']) && !empty($log['emails']) && !empty($settings['shd_notify_log']))
		wesql::insert('',
...
function shd_notify_popup()
{
	...
	if (empty($settings['shd_display_ticket_logs']) || empty($_GET['log']) || empty($email_type))
	...
	if (!empty($users))
	{
		...
		foreach ($users as $user)
		{
			...
			$new_content .= ($first ? '<img src="' . $context['plugins_url']['Arantor:WedgeDesk'] . '/images/user.png"> ' : ', ') . shd_profile_link($user_profile[$user]['real_name'], $user);
			$first = false;
		}
	}
	...
	if (!empty($row['extra']['emails'][$email_type]['e']))
	{
		...
		if (shd_allowed_to('admin_helpdesk', 0) || we::$is_admin)
		{
			foreach ($emails as $key => $value)
				$emails[$key] = '<a href="mailto:' . $value . '">' . $value . '</a>';
			$new_content .= '<img src="' . $context['plugins_url']['Arantor:WedgeDesk'] . '/images/log_notify.png"> ' . implode(', ', $emails);
		}
		// No-one else can at the moment.
		else
			$new_content .= '<img src="' . $context['plugins_url']['Arantor:WedgeDesk'] . '/images/log_notify.png"> ' . (count($emails) == 1 ? $txt['shd_log_notify_hiddenemail_1'] : sprintf($txt['shd_log_notify_hiddenemail'], count($emails)));
	}
	...
	$replacements = array(
		"\n" => '<br>',
		'{ticket_id}' => str_pad($row['id_ticket'], $settings['shd_zerofill'], '0', STR_PAD_LEFT),
		'{subject}' => empty($row['extra']['subject']) ? $txt['no_subject'] : $row['extra']['subject'],
		'{ticketlink}' => '<URL>?action=helpdesk;sa=ticket;ticket=' . $row['id_ticket'] . (empty($row['id_msg']) ? '.0' : '.msg' . $row['id_msg'] . '#msg' . $row['id_msg']),
		'{body}' => $body,
...
function shd_notify_ticket_options()
{
	...
	if (wesql::num_rows($query) == 0)
		...
		list($old_state) = wesql::fetch_row($query);
	wesql::free_result($query);
