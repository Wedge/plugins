...
function shd_scheduled_close_tickets()
{
	...
	if (!empty($tickets))
	{
		...
		if (empty($settings['shd_disable_action_log']) && !empty($settings['shd_logopt_autoclose']))
		{
			$rows = array();
...
function shd_scheduled_purge_tickets()
{
	...
	if (empty($settings['shd_autopurge_tickets']) || empty($settings['shd_autopurge_tickets_days']))
		return;
	...
	$del_time = time() - (86400 * $settings['shd_autopurge_tickets_days']);
