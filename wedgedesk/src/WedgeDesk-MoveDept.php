...
function shd_movedept()
{
	...
	$context['can_pm'] = empty($settings['shd_helpdesk_only']) || empty($settings['shd_disable_pm']);
	...
	if (shd_allowed_to('shd_move_dept_any', $context['current_dept']) || (shd_allowed_to('shd_move_dept_own', $context['current_dept']) && $ticket_starter == we::$id))
	{
		...
		$context['linktree'][] = array(
			'url' => '<URL>?' . $context['shd_home'] . ';dept=' . $context['current_dept'],
		...
		loadPluginTemplate('Arantor:WedgeDesk', 'tpl/WedgeDesk-MoveDept');
		wetem::load('movedept');
	}
	else
...
function shd_movedept2()
{
	...
	if ((isset($_POST['send_pm']) && (!isset($_POST['pm_content']) || trim($_POST['pm_content']) == '')) && (empty($settings['shd_helpdesk_only']) || empty($settings['shd_disable_pm'])))
	...
	if (shd_allowed_to('shd_move_dept_any', $context['current_dept']) || (shd_allowed_to('shd_move_dept_own', $context['current_dept']) && $ticket_starter == we::$id))
	{
		...
		list($new_dept, $dept_name) = wesql::fetch_row($query);
		wesql::free_result($query);
		...
		call_hook('shd_hook_movedept', array(&$new_dept, &$dept_name));
		...
		if (isset($_POST['send_pm']))
		{
			...
			$replacements = array(
				...
				'{new_dept}' => $dept_name,
				'{link}' => '<URL>?action=helpdesk;sa=ticket;ticket=' . $context['ticket_id'],
			);
