 ...
 * WedgeDesk
 *
 * This file handles the core permissions systems for WedgeDesk, including the permissions templates, loading and checking permissions.
 *
 * @package wedgedesk
...
function shd_load_all_permission_sets()
{
	...
	// Each item = array(bool has-own/any, permission category, image file in $context['plugins_url']['Arantor:WedgeDesk'] . '/images/')
	...
	if (!empty($settings['shd_disable_tickettotopic']))
...
function shd_load_role_templates()
{
	...
	call_hook('shd_hook_permstemplate');
}

/**
 *	Defines user permissions, most importantly concerning ticket visibility
 *
 *	Populates specific parameters in we::$user, mostly to add {} query parameters.
...
function shd_load_user_perms()
{
	...
	if (!empty(we::$user['query_see_ticket']))
		return;
	...
	if (we::$is_guest)
	{
		we::$user['shd_permissions'] = array();
		we::$user['query_see_ticket'] = '1=0';
	}
	elseif (!we::$is_admin)
	{
		$permissions_cache = 'shd_permissions_' . implode('-', we::$user['groups']);
		$perm_cache_time = 300;
		$temp = cache_get_data($permissions_cache, $perm_cache_time);

		if ($temp === null || (time() - $perm_cache_time > $settings['settings_updated']))
		{
				...
				foreach ($role_permissions as $role => $perm_list)
				{
					...
					foreach ($perm_list as $perm => $value)
					{
						if ($value == ROLEPERM_ALLOW)
							we::$user['shd_permissions'][$perm] = $depts[$role];
					}
				}
			...
			if (!empty($denied))
			{
				...
				{
					if (isset(we::$user['shd_permissions'][$perm]))
						unset(we::$user['shd_permissions'][$perm]);
				}
			}

			cache_put_data($permissions_cache, we::$user['shd_permissions'], $perm_cache_time);
		}
		else
			we::$user['shd_permissions'] = $temp;
	}
	elseif (we::$is_admin)
	{
	...
	if (we::$is_admin)
		we::$user['query_see_ticket'] = '1=1';
	elseif (!shd_allowed_to('access_helpdesk', 0))
		we::$user['query_see_ticket'] = '1=0'; // no point going any further if they can't access the helpdesk
	else
	{
		...
		// Finally, assemble into we::$user.
		if (empty($clauses))
			we::$user['query_see_ticket'] = '1=0';
		else
			we::$user['query_see_ticket'] = '((' . implode(') AND (', $clauses) . '))';
	}
 ...
 *
 *	All WedgeDesk-specific permissions should be checked with this function. Any other permission check that is not specifically
 *	WedgeDesk-related should use allowedTo instead.
 *
...
function shd_allowed_to($permission, $dept = 0)
{
	...
	if ($dept === false)
	{
		if (we::$is_admin)
			...
			return empty(we::$user['shd_permissions'][$permission]) ? array() : we::$user['shd_permissions'][$permission];
	}
	elseif ($dept == 0)
	{
		if (!empty(we::$is_admin))
			return true;
		elseif (!is_array($permission) && !empty(we::$user['shd_permissions'][$permission]))
			return true;
		elseif (is_array($permission))
		{
			foreach ($permission as $perm)
				if (!empty(we::$user['shd_permissions'][$perm]))
					return true;
	...
	else
	{
		...
		foreach ($permission as $perm)
			if (!empty(we::$user['shd_permissions'][$perm]) && in_array($dept, we::$user['shd_permissions'][$perm]))
				return true;
 ...
 *
 *	All fatal-level WedgeDesk-specific permissions should be checked with this function. Any other permission check that is
 *	not specifically WedgeDesk-related should use isAllowedTo instead. Note that this is a void function because should this
 *	fail, Wedge execution will be halted.
 *
 ...
 *	an internal lookup provided by Wedge.
 *
...
/**
 *	Identifies which membergroups hold a given permission.
 *
