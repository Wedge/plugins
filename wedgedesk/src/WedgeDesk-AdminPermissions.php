 ...
 * WedgeDesk
 *
 * This file handles the core of WedgeDesk's permissions system.
 *
 * @package wedgedesk
...
/**
 *	This function is the start point for configuration of permissions within WedgeDesk.
 *
 *	@since 2.0
*/
function shd_admin_permissions()
{
	...
	loadPluginTemplate('Arantor:WedgeDesk', 'tpl/WedgeDesk-AdminPermissions');
...
function shd_admin_role_list()
{
	...
	wetem::load('shd_permissions_home');
...
function shd_admin_create_role()
{
	...
	else
	{
		...
		if (!isset($_POST['rolename']) || westr::htmltrim(westr::htmlspecialchars($_POST['rolename'])) === '')
			fatal_lang_error('shd_no_role_name', false);
		else
			$_POST['rolename'] = strtr(westr::htmlspecialchars($_POST['rolename']), array("\r" => '', "\n" => '', "\t" => ''));
...
function shd_admin_save_role()
{
	...
	// 8. (serious voice) OK let's do groups. Grab the ones that are valid groups in Wedge, ignore everything else
	// We're not interested in admin (group 1), board mod (group 3) or post count groups (min_posts != -1)
...
function shd_load_role($loadrole = 0)
{
	...
	while ($row = wesql::fetch_assoc($query))
	{
		...
		$context['shd_permissions']['user_defined_roles'][$row['id_role']]['groups'][$row['id_group']] = array(
			'name' => $row['group_name'],
			'color' => $row['online_color'],
			'link' => empty($row['id_group']) ? $row['group_name'] : '<a href="<URL>?action=groups;sa=members;group=' . $row['id_group'] . '"' . (empty($row['online_color']) ? '' : ' style="color: ' . $row['online_color'] . ';"') . '>' . $row['group_name'] . '</a>',
		);
	}
	wesql::free_result($query);
