 ...
 * WedgeDesk
 *
 * This file handles the core of WedgeDesk's administrative maintenance.
 *
 * @package wedgedesk
...
function shd_admin_maint()
{
	...
	$context[$context['admin_menu_name']]['tab_data'] = array(
		'title' => '<img src="' . $context['plugins_url']['Arantor:WedgeDesk'] . '/images/' . $subactions[$_REQUEST['sa']]['icon'] . '" class="icon">' . $subactions[$_REQUEST['sa']]['title'],
...
function shd_admin_maint_home()
{
	...
	wetem::load('shd_admin_maint_home');
...
function shd_admin_maint_reattribute()
{
	...
	wetem::load('shd_admin_maint_reattributedone');
...
function shd_admin_maint_massdeptmove()
{
	...
	{
		wetem::load('shd_admin_maint_massdeptmovedone');
		return;
	}
...
function shd_admin_maint_findrepair()
{
	...
	if (isset($_GET['done']))
	{
		wetem::load('shd_admin_maint_findrepairdone');
...
function shd_maint_zero_entries()
{
	...
	$query = wesql::query('
		...
		FROM {db_prefix}helpdesk_tickets
		WHERE id_ticket = 0');
	list($tickets) = wesql::fetch_row($query);
	if (!empty($tickets))
	{
		wesql::query('
			UPDATE {db_prefix}helpdesk_tickets
			SET id_ticket = NULL
			WHERE id_ticket = 0');
		$_SESSION['shd_maint']['zero_tickets'] = wesql::affected_rows();
	}
	...
	$query = wesql::query('
		...
		FROM {db_prefix}helpdesk_ticket_replies
		WHERE id_msg = 0');
	list($msgs) = wesql::fetch_row($query);
	if (!empty($msgs))
	{
		wesql::query('
			UPDATE {db_prefix}helpdesk_ticket_replies
			SET id_msg = NULL
			WHERE id_msg = 0');
		$_SESSION['shd_maint']['zero_msgs'] = wesql::affected_rows();
	}
...
function shd_maint_deleted()
{
	...
	$query = wesql::query('
		SELECT COUNT(*)
		FROM {db_prefix}helpdesk_tickets');
	list($ticket_count) = wesql::fetch_row($query);
	wesql::free_result($query);
...
// Make sure all WedgeDesk cache items are forcibly flushed.
...
function shd_admin_maint_search()
{
	...
	wetem::load('shd_admin_maint_search');
	$context['page_title'] = $txt['shd_admin_maint'];
	...
	if (isset($_REQUEST['rebuild']))
	{
		...
		$query = wesql::query('
			...
			FROM {db_prefix}helpdesk_tickets');
		list($total) = wesql::fetch_row($query);
	...
	if (isset($_REQUEST['save']))
	{
		...
		$normal_regex = shd_return_exclude_regex($settings['shd_search_charset']);
		if (empty($_POST['shd_search_charset']))
			$_POST['shd_search_charset'] = $settings['shd_search_charset'];
...
// This uses the same methodology as Subs-WedgeDeskSearch.php's shd_search_charset routine.
