 ...
 * WedgeDesk
 *
 * This file handles data gathering primarily for SSI.php purposes. It expects Subs-WedgeDesk.php to have been required as this
 ...
 *
 * Unlike other WedgeDesk functions, these use a SSI style camel casing.
 *
 * @package wedgedesk
...
function ssi_getSDTickets($query_where, $query_where_params = array(), $query_limit = 0, $query_order = 'hdt.id_ticket ASC', $output_method = 'echo')
{
	...
	while ($row = wesql::fetch_assoc($query))
	{
		...
		$tickets[] = array(
			...
			'href' => '<URL>?action=helpdesk;sa=ticket;ticket=' . $row['id_ticket'],
			'opener' => array(
				'id' => $row['starter_id'],
 ...
 *	@return array The return is always an array of members that are staff; contains many details about members since Wedge's member context is loaded (including avatar, personal text and so on)
 *	@since 2.0
*/
function ssi_staffMembers($honour_admin_setting = true, $output_method = 'echo')
{
	...
	if ($honour_admin_setting && !empty($settings['shd_admins_not_assignable']))
	{
		$admins = array();
		$query = wesql::query('
			SELECT id_member
