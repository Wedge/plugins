 ...
 * WedgeDesk
 *
 * This file handles probably the two most critical functions in WedgeDesk: the one that adds new posts to the database
 ...
 *
 *	This function handles all of the creation of posts and tickets within WedgeDesk, even with respect to managing tickets spawned
 ...
 *	<li>body: Required string, the principle body content of the message to post. Assumed to have been cleaned already (with westr::htmlspecialchars and preparsecode)</li>
 ...
 *	<li>subject: Semi-optional string with the new subject in; required for a new ticket, ignored if adding a reply. If set, assumed to have been cleaned already (with westr::htmlspecialchars and strtr)</li>
...
function shd_create_ticket_post(&$msgOptions, &$ticketOptions, &$posterOptions)
{
	...
	{
		// We need to get the number of posts, and the highest one already in this ticket (regardless of their status).
		$query = wesql::query('
			SELECT next_reply
		...
		if ($row = wesql::fetch_row($query))
			...
			$reply_num = 0; // This shouldn't really happen but you never can tell.
	...
	wesql::insert('',
		...
		array(
			...
			'smileys_enabled' => 'int', 'modified_member' => 'int', 'modified_name' => 'string', 'modified_time' => 'int', 'reply_num' => 'int',
		),
		array(
			$ticketOptions['id'], $posterOptions['id'], $msgOptions['body'],
			$posterOptions['name'], $posterOptions['email'], $msgOptions['time'], get_ip_identifier($posterOptions['ip']),
			$msgOptions['smileys_enabled'] ? 1 : 0, $msgOptions['modified']['id'], $msgOptions['modified']['name'], $msgOptions['modified']['time'], $reply_num,
		),
		array('id_msg')
	);
	...
	if ($new_ticket)
	{
		wesql::insert('',
			...
			array(
				...
				'status' => 'int', 'private' => 'int', 'next_reply' => 'int',
			),
			array(
				...
				$ticketOptions['private'] ? 1 : 0, $reply_num + 1,
			),
			array('id_ticket')
		);
		$ticketOptions['id'] = wesql::insert_id();
	...
	call_hook($hook, array(&$msgOptions, &$ticketOptions, &$posterOptions));
	...
	if (empty($ticketOptions['dept']) && !empty($ticketOptions['id']))
	{
		...
		list($ticketOptions['dept']) = wesql::fetch_row($query);
		wesql::free_result($query);
	}
	...
	if (empty($context['shd_no_search']))
	{
		...
		loadPluginSource('Arantor:WedgeDesk', 'src/Subs-WedgeDeskSearch');
 ...
 *	in WedgeDesk) to update just a ticket or just a post from this function. All three parameters are by <b>reference</b>
 ...
 *	<li>body: Optional, the principle body content of the message to change; if omitted, no change will occur. If set, assumed to have been cleaned already (with westr::htmlspecialchars and preparsecode)</li>
 *	<li>modified: Optional, hash array containing items relating to modification (if 'modified' key exists, all of these should be set)
 *		<ul>
 *			<li>time: Unsigned int timestamp of the change</li>
 ...
 *	<li>subject: Optional string with the new subject in; if omitted no change will occur. If set, assumed to have been cleaned already (with westr::htmlspecialchars and strtr)</li>
...
function shd_modify_ticket_post(&$msgOptions, &$ticketOptions, &$posterOptions)
{
	...
	// Int hook
	call_hook('shd_hook_modpost', array(&$msgOptions, &$ticketOptions, &$posterOptions));
 ...
 *	format used in the ticket posting/reply posting functions in WedgeDesk-Post.php.
 *
...
function shd_load_custom_fields($is_ticket = true, $ticketContext = 0, $dept = 0)
{
	...
	while($row = wesql::fetch_assoc($custom_fields))
	{
		...
		if (isset($field_values[$row['id_field']]))
		{
			...
			if ($context['ticket_form']['custom_fields'][$loc][$row['id_field']]['type'] == CFIELD_TYPE_TEXT || $context['ticket_form']['custom_fields'][$loc][$row['id_field']]['type'] == CFIELD_TYPE_LARGETEXT)
				$context['ticket_form']['custom_fields'][$loc][$row['id_field']]['value'] = wedit::un_preparsecode($context['ticket_form']['custom_fields'][$loc][$row['id_field']]['value']);
		}
	}
			...
			switch ($field['type'])
			{
				...
				case CFIELD_TYPE_LARGETEXT:
					...
					else
					{
						...
						$value = westr::htmlspecialchars($value, ENT_QUOTES);
