 ...
 * WedgeDesk
 *
 * This file is one of the cornerstones of WedgeDesk; it handles displaying the post form to users, both for tickets and replies,
...
function shd_post_ticket()
{
	...
	// Things we need
	loadSource(array('Class-Editor', 'Subs-Post'));
	...
	$context['ticket_form'] = array( // yes, everything goes in here.
		...
		'form_action' => '<URL>?action=helpdesk;sa=saveticket',
		...
		'link' => $new_ticket ? '' : '<a href="<URL>?action=helpdesk;sa=ticket;ticket=' . $context['ticket_id'] . '">' . $ticketinfo['subject'] . '</a>',
		'msg' => $new_ticket ? 0 : $ticketinfo['id_first_msg'],
		'display_id' => $new_ticket ? '' : str_pad($context['ticket_id'], $settings['shd_zerofill'], '0', STR_PAD_LEFT),
		...
		'num_allowed_attachments' => empty($settings['attachmentNumPerPostLimit']) || $settings['shd_attachments_mode'] == 'ticket' ? -1 : $settings['attachmentNumPerPostLimit'],
	...
	else
	{
		...
		$context['ticket_form']['message'] = wedit::un_preparsecode($context['ticket_form']['message']);
	...
	// Add the things that we're going to need in the posting template.

	shd_ticket_form_setup();

	// Register this form in the session variables.
	checkSubmitOnce('register');
}

// All the template layers required for the ticket post form.
function shd_ticket_form_setup()
{
	global $context;
	...
	wetem::load(
		array(
			...
			'ticket_postbox',
			'ticket_footer',
			'ticket_replies' => array('ticket_do_replies'),
			'ticket_pageend',
			'ticket_proxy_js',
		),
		'default',
	...
	if (empty($context['ticket_form']['do_replies']))
		wetem::remove('ticket_replies');
}

function shd_reply_form_setup()
{
	global $context;
...
function shd_save_post()
{
	...
	if (!empty($_REQUEST['shd_message_mode']) && isset($_REQUEST['shd_message']))
	{
		...
		wedit::preparseWYSIWYG('shd_message');
	}
	...
	if (!isset($_POST['shd_message']) || westr::htmltrim(westr::htmlspecialchars($_POST['shd_message']), ENT_QUOTES) === '')
	{
	...
	{
		$_POST['shd_message'] = westr::htmlspecialchars($_POST['shd_message'], ENT_QUOTES);
		wedit::preparsecode($_POST['shd_message']);

		// Make sure there's something underneath all the tags
		if (westr::htmltrim(strip_tags(shd_format_text($_POST['shd_message'], false), '<img>')) === '' && (!allowedTo('admin_forum') || strpos($_POST['shd_message'], '[html]') === false))
			$context['shd_errors'][] = 'no_message';
		elseif (!empty($settings['max_messageLength']) && westr::strlen($_POST['shd_message']) > $settings['max_messageLength'])
		{
			$context['shd_errors'][] = 'long_message';
			$txt['error_long_message'] = sprintf($txt['error_long_message'], $settings['max_messageLength']);
		}
	}
...
function shd_save_ticket()
{
	...
	$context['ticket_form'] = array(
		...
		'link' => $new_ticket ? '' : '<a href="<URL>?action=helpdesk;sa=ticket;ticket=' . $context['ticket_id'] . '">' . $ticketinfo['subject'] . '</a>',
		'msg' => $msg,
		'display_id' => empty($context['ticket_id']) ? '' : str_pad($context['ticket_id'], $settings['shd_zerofill'], '0', STR_PAD_LEFT),
		'status' => $new_status,
		'private' => array(
			'setting' => $private,
	...
	if ($context['can_post_proxy'] && !empty($_REQUEST['proxy_author']))
	{
		...
		$proxy_author = westr::htmlspecialchars(westr::strtolower(trim($_REQUEST['proxy_author'])));
		$_REQUEST['proxy_author'] = westr::htmlspecialchars(trim($_REQUEST['proxy_author']));
	...
	if (!empty($context['shd_errors']) || !empty($context['ticket_form']['preview'])) // Uh oh, something went wrong, or we're previewing
	{
		...
		shd_postbox(
			'shd_message',
			wedit::un_preparsecode($context['ticket_form']['message']),
			array(
				'post_button' => $context['ticket_form']['form_title'],
			)
		);
...
function shd_post_reply()
{
	...
	else
	{
		...
		$reply['body'] = wedit::un_preparsecode($reply['body']);
	...
	$context['ticket_form'] = array( // yes, everything goes in here.
		...
		'form_action' => '<URL>?action=helpdesk;sa=savereply',
		...
		'ticket_link' => '<a href="<URL>?action=helpdesk;sa=ticket;ticket=' . $context['ticket_id'] . '">' . $ticketinfo['subject'] . '</a>',
		'reply_link' => $new_reply ? 0 : '<a href="<URL>?action=helpdesk;sa=ticket;ticket=' . $context['ticket_id'] . '.msg' . $reply['id_msg'] . '#msg' . $reply['id_msg'] . '">' . $txt['response_prefix'] . ' ' . $ticketinfo['subject'] . '</a>',
...
function shd_save_reply()
{
	...
	if (!empty($context['shd_errors']) || !empty($context['ticket_form']['preview'])) // Uh oh, something went wrong, or we're previewing
	{
		...
		shd_postbox(
			'shd_message',
			wedit::un_preparsecode($_POST['shd_message']),
			array(
...
function shd_done_posting()
{
	...
	$thank_you = false;
	if (!empty($settings['shd_thank_you_post']) && empty($context['ticket_form']['msg']))
	{
		...
		if (empty($settings['shd_thank_you_nonstaff']) || !shd_allowed_to('shd_staff', $context['shd_department']))
		{
			$thank_you = true;
			wetem::load('shd_thank_posting', 'default', 'add');
			...
			$replacements = array(
				'{membername}' => we::$user['name'],
				'{subject}' => $context['ticket_form']['subject'],
				'{ticketurl}' => '<URL>?action=helpdesk;sa=ticket;ticket=' . $context['ticket_id'],
				'{forum_name}' => $context['forum_name_html_safe'],
				'{newticketlink}' => '<URL>?action=helpdesk;sa=newticket',
				'{helpdesklink}' => '<URL>?' . $context['shd_home'] . $context['shd_dept_link'],
				'{forumlink}' => '<URL>',
	...
	if (!empty($messages))
	{
		...
		$reply_request = wesql::query('
			SELECT
				...
				smileys_enabled, poster_name, poster_email, reply_num
			FROM {db_prefix}helpdesk_ticket_replies
...
function shd_postbox($id, $message, $buttons, $width = '70%')
{
	...
	$context['postbox'] = new wedit(
		array(
			...
			'width' => $width,
			'drafts' => 'none',
			'disable_smiley_box' => empty($settings['shd_allow_ticket_bbc']) || $context['ticket_form']['disable_smileys'],
			'custom_bbc_div' => 'shd_bbcbox',
			'custom_smiley_div' => 'shd_smileybox',
		)
	);
	...
	if (!empty($settings['attachmentNumPerPostLimit']) && $settings['shd_attachments_mode'] != 'ticket')
		$context['attachment_restrictions'][] = sprintf($txt['attach_restrict_attachmentNumPerPostLimit'], $settings['attachmentNumPerPostLimit']);

	if (!empty($settings['attachmentPostLimit']) && $settings['shd_attachments_mode'] != 'ticket')
		$context['attachment_restrictions'][] = sprintf($txt['attach_restrict_attachmentPostLimit'], $settings['attachmentPostLimit']);

	if (!empty($settings['attachmentSizeLimit']))
		$context['attachment_restrictions'][] = sprintf($txt['attach_restrict_attachmentSizeLimit'], $settings['attachmentSizeLimit']);
}
...
function shd_load_attachments()
{
	...
	$query = wesql::query('
		...
		WHERE ' . ($settings['shd_attachments_mode'] == 'ticket' ? 'hda.id_ticket = {int:ticket}' : 'hda.id_msg = {int:msg}') . '
...
function shd_check_attachments()
{
	...
	if (shd_allowed_to('shd_post_attachment', $context['ticket_form']['dept']))
	{
		...
		if (($settings['shd_attachments_mode'] == 'ticket' && !empty($context['ticket_id'])) || isset($context['ticket_form']['msg']))
		{
			$request = wesql::query('
		...
		if (!empty($_FILES))
		{
			...
			foreach ($_FILES as $uplfile)
			{
				...
				if (!empty($settings['attachmentSizeLimit']) && $uplfile['size'] > $settings['attachmentSizeLimit'] * 1024)
				{
					checkSubmitOnce('free');
				...
				$quantity++;
				if (!empty($settings['attachmentNumPerPostLimit']) && $quantity > $settings['attachmentNumPerPostLimit'] && $settings['shd_attachments_mode'] != 'ticket')
				{
					checkSubmitOnce('free');
				...
				{
					if (!in_array(strtolower(substr(strrchr($uplfile['name'], '.'), 1)), explode(',', strtolower($settings['attachmentExtensions']))))
					{
						checkSubmitOnce('free');
						fatal_error($uplfile['name'] . '.<br>' . $txt['cant_upload_type'] . ' ' . $settings['attachmentExtensions'] . '.', false);
					}
				}
...
function shd_handle_attachments()
{
	...
	if (!empty($_FILES) || !empty($_SESSION['temp_attachments']))
	{
		...
		foreach ($_FILES as $uplfile)
		{
			...
			$file_limit = (!empty($settings['attachmentNumPerPostLimit']) && $settings['shd_attachments_mode'] != 'ticket') ? $settings['attachmentNumPerPostLimit'] : $quantity + 1;
			...
			$total_size += $uplfile['size'];
			$size_limit = (!empty($settings['attachmentPostLimit']) && $settings['shd_attachments_mode'] != 'ticket') ? $settings['attachmentPostLimit'] * 1024 : $total_size + 1024;
			...
			else
			{
				...
				{
					checkSubmitOnce('free');
					fatal_error($attachmentOptions['name'] . '.<br>' . $txt['cant_upload_type'] . ' ' . strtr($settings['attachmentExtensions'], array(',' => ', ')) . '.', false);
				}
...
function shd_posting_additional_options()
{
	...
	$context['ticket_form']['additional_opts'] = array(
		'goback' => array(
			...
			'show' => (empty($context['ticket_form']['ticket']) && !empty($settings['shd_thank_you_post']) && (empty($settings['shd_thank_you_nonstaff']) || !shd_allowed_to('shd_staff', 0))) ? false : true,
		),
		'no_smileys' => array(
			'checked' => !empty($context['ticket_form']['disable_smileys']),
			'text' => $txt['shd_disable_smileys_post'],
			'show' => !empty($settings['shd_allow_ticket_smileys']),
		),
...
function shd_load_canned_replies()
{
	...
	if (we::$is_admin || shd_allowed_to('admin_helpdesk', $context['shd_department']))
		$visible = '';
