...
function shd_tickettotopic()
{
	...
	if (!shd_allowed_to('shd_ticket_to_topic', $dept) || !empty($settings['shd_helpdesk_only']) || !empty($settings['shd_disable_tickettotopic']))
	...
	if ($context['shd_multi_dept'])
	{
		$context['linktree'][] = array(
			'url' => '<URL>?' . $context['shd_home'] . ';dept=' . $dept,
			'name' => $dept_name,
		);
	}
	...
	// Build the linktree
	$context['linktree'][] = array(
		'url' => '<URL>?' . $context['shd_home'] . ($context['shd_multi_dept'] ? ';dept=' . $dept : ''),
	...
	loadPluginTemplate('Arantor:WedgeDesk', 'tpl/WedgeDesk-TicketTopicMove');
	wetem::load('shd_tickettotopic');
 ...
 *
 *	This is done by invoking Wedge's createPost function to make the new thread and repost all the ticket's posts as new thread posts
...
function shd_tickettotopic2()
{
	...
	if (wesql::num_rows($request) == 0)
		...
		list ($pcounter, $board_name, $subject, $owner, $body, $firstmsg, $smileys_enabled, $modified_time, $modified_name, $time, $shd_id_msg, $deleted_replies, $dept) = wesql::fetch_row($request);
	...
	call_hook('shd_hook_tickettotopic');
	...
	// Keep track of SHD msg id to forum msg id
	...
	if (isset($topicOptions['id']))
	{
		...
		if (isset($_POST['send_pm']))
		{
			...
			list ($userid, $username) = wesql::fetch_row($request);
			wesql::free_result($request);
...
function shd_topictoticket()
{
	...
	if (!shd_allowed_to('shd_topic_to_ticket', 0) || !empty($settings['shd_helpdesk_only']) || !empty($settings['shd_disable_tickettotopic']))
	...
	$context['linktree'][] = array(
		'url' => '<URL>?topic=' . $context['topic_id'] . '.0',
		'name' => $subject,
	);
...
function shd_topictoticket2()
{
	...
	if (!shd_allowed_to('shd_topic_to_ticket', $_REQUEST['dept']) || !empty($settings['shd_helpdesk_only']) || !empty($settings['shd_disable_tickettotopic']))
		fatal_lang_error('shd_cannot_move_topic', false);
	...
	call_hook('shd_hook_topictoticket');
	...
	$msg_assoc[$forum_id_msg] = $msgOptions['id'];
	...
	if (isset($ticketOptions['id']))
	{
		...
		$num_replies = wesql::num_rows($request) + 1; // Plus one since we want to count the main ticket post as well.
		...
		if (isset($_POST['send_pm']))
		{
			...
			$replacements = array(
				'{user}' => $username,
				'{subject}' => $old_subject,
				'{link}' => '<URL>?action=helpdesk;sa=ticket;ticket=' . $ticket,
			);
			$message = str_replace(array_keys($replacements), array_values($replacements), $_POST['pm_content']);
		...
		if (!empty($msg_assoc))
		{
			...
			$query = wesql::query('
				...
				WHERE id_msg IN ({array_int:forum_msgs})',
				array(
					'forum_msgs' => array_keys($msg_assoc),
				)
			);
			...
			if (!empty($attachIDs))
			{
				...
				// 2.2. "Remove" them from Wedge's table
				wesql::query('
					UPDATE {db_prefix}attachments
					SET id_msg = 0
					WHERE id_msg IN ({array_int:forum_msgs})',
