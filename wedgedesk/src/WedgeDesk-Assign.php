...
function shd_assign()
{
	...
	if (shd_allowed_to('shd_assign_ticket_any', $dept)) // can regularly assign? If so, load up potential candidates and throw it at the template.
	{
		...
		loadPluginTemplate('Arantor:WedgeDesk', 'tpl/WedgeDesk-Assign');
		wetem::load('assign');
	}
...
function shd_commit_assignment($ticket, $assignment, $is_ajax = false)
{
	...
	loadPluginSource('Arantor:WedgeDesk', array('src/Subs-WedgeDeskPost', 'src/WedgeDesk-Notifications'));
	...
	call_hook('shd_hook_assign', array(&$ticket, &$assignment));
...
function shd_get_possible_assignees($private = false, $ticket_owner = 0, $dept = -1)
{
	...
	if (!empty($settings['shd_admins_not_assignable']))
	{
		$query = wesql::query('
			SELECT id_member
			FROM {db_prefix}members
	...
	if (empty($settings['shd_staff_ticket_self'])) // by default, staff members can't be assigned a ticket if they started it
