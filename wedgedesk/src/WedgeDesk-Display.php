 ...
 *			</ul>
 *		</li>
 *		<li>define the page index with Wedge's constructPageIndex</li>
 ...
 *		<li>call in the editor component from WedgeDesk-Post.php and friends, ready for Quick Reply</li>
...
{
	global $context, $txt, $reply_request, $settings, $memberContext, $options;

	loadPluginTemplate('Arantor:WedgeDesk', 'tpl/WedgeDesk-Display');
	wetem::load(
		array(
			'shd_display_nojs',
			'display_main_navigation',
			'display_ticket_header',
			'viewticket',
			'additional_fields',
			'viewreplies',
			'quickreply',
	...
	$context['ticket'] = array(
		...
		'display_id' => str_pad($context['ticket_id'], $settings['shd_zerofill'], '0', STR_PAD_LEFT),
	...
	if (shd_allowed_to('shd_view_ip_any', $context['ticket']['dept']) || ($context['ticket']['ticket_opener'] && shd_allowed_to('shd_view_ip_own', $context['ticket']['dept'])))
		$context['ticket']['ip_address'] = $context['link_ip_address'] ? ('<a href="<URL>?action=trackip;searchip=' . $ticketinfo['starter_ip'] . '">' . $ticketinfo['starter_ip'] . '</a>') : $ticketinfo['starter_ip'];
	...
	elseif ($context['ticket']['deleted_replies'] > 0)
	{
		if (shd_allowed_to('shd_access_recyclebin', $context['ticket']['dept']))
		{
			...
			$ticketlink = '<URL>?action=helpdesk;sa=ticket;ticket=' . $context['ticket_id'] . (isset($_REQUEST['recycle']) ? '' : ';recycle');
	...
	if (empty($settings['shd_privacy_display']) || $settings['shd_privacy_display'] == 'smart')
	...
	$context['page_index'] = shd_no_expand_pageindex('<URL>?action=helpdesk;sa=ticket;ticket=' . $context['ticket_id'] . '.%1$d' . (isset($_REQUEST['recycle']) ? ';recycle' : '') . '#replies', $context['ticket_start_from'], $context['total_visible_posts'], $context['messages_per_page'], true);
	...
	{
		$reply_request = wesql::query('
			SELECT
				id_msg, poster_time, li.member_ip AS poster_ip, id_member, modified_time, modified_name, modified_member, body,
				smileys_enabled, poster_name, poster_email, message_status, reply_num
	...
	$context['canonical_url'] = '<URL>?action=helpdesk;sa=ticket;ticket=' . $context['ticket_id'];
	...
	$context['linktree'][] = array(
		'url' => '<URL>?action=helpdesk;sa=ticket;ticket=' . $context['ticket_id'],
		'name' => $ticketname,
	);
	...
	$context['can_quote'] = $context['can_reply'] && !empty($settings['shd_allow_ticket_bbc']);
	$context['can_go_advanced'] = !empty($settings['shd_allow_ticket_bbc']) || !empty($settings['allow_ticket_smileys']) || shd_allowed_to('shd_post_attachment', $context['ticket']['dept']);
	$context['shd_can_move_to_topic'] = empty($settings['shd_disable_tickettotopic']) && shd_allowed_to('shd_ticket_to_topic', $context['ticket']['dept']) && empty($settings['shd_helpdesk_only']);
	...
	$context['ticket_navigation'][] = array(
		'url' => '<URL>?action=helpdesk;sa=editticket;ticket=' . $context['ticket']['id'] . ';' . $context['session_query'],
		'icon' => 'edit',
	...
	$context['ticket_navigation'][] = array(
		'url' => '<URL>?action=helpdesk;sa=markunread;ticket=' . $context['ticket']['id'] . ';' . $context['session_query'],
		'icon' => 'unread',
		'display' => !$context['ticket']['closed'] && !$context['ticket']['deleted'],
		'text' => 'shd_ticket_markunread',
	);
	$context['ticket_navigation'][] = array(
		'url' => '<URL>?action=helpdesk;sa=resolveticket;ticket=' . $context['ticket']['id'] . ';' . $context['session_query'],
		'icon' => 'resolved',
	...
	$assign_nav = array(
		'url' => '<URL>?action=helpdesk;sa=assign;ticket=' . $context['ticket']['id'] . ';' . $context['session_query'],
		'icon' => 'assign',
		'text' => '',
		'display' => false,
	);
	...
	$context['ticket_navigation'][] = array(
		'url' => '<URL>?action=helpdesk;sa=deleteticket;ticket=' . $context['ticket']['id'] . ';' . $context['session_query'],
		'icon' => 'delete',
	...
	$context['ticket_navigation'][] = array(
		'url' => '<URL>?action=helpdesk;sa=restoreticket;ticket=' . $context['ticket']['id'] . ';' . $context['session_query'],
		'icon' => 'restore',
	...
	$context['ticket_navigation'][] = array(
		'url' => '<URL>?action=helpdesk;sa=permadelete;ticket=' . $context['ticket']['id'] . ';' . $context['session_query'],
		'icon' => 'delete',
	...
	$context['ticket_navigation'][] = array(
		'url' => '<URL>?action=helpdesk;sa=movedept;ticket=' . $context['ticket']['id'] . ';' . $context['session_query'],
		'icon' => 'movedept',
		'display' => $context['can_move_dept'],
		'text' => 'shd_move_dept',
	);
	$context['ticket_navigation'][] = array(
		'url' => '<URL>?action=helpdesk;sa=tickettotopic;ticket=' . $context['ticket']['id'] . ';' . $context['session_query'],
		'icon' => 'tickettotopic',
	...
	loadPluginSource('Arantor:WedgeDesk', 'src/WedgeDesk-Post');
	loadPluginTemplate('Arantor:WedgeDesk', 'tpl/WedgeDesk-Post');

	$context['ticket_form']['ticket'] = $context['ticket_id'];
	$context['ticket_form']['num_allowed_attachments'] = empty($settings['attachmentNumPerPostLimit']) || $settings['shd_attachments_mode'] == 'ticket' ? -1 : $settings['attachmentNumPerPostLimit'];
	...
	$context['ticket_form']['disable_smileys'] = empty($settings['shd_allow_ticket_smileys']);
	...
	if ($context['ticket']['privacy']['can_change'])
		add_js('
	var ajaxPrivacy = new AjaxSelector({
		sSelf: "ajaxPrivacy",
		sListItem: "item_privacy",
		sQueryUrl: "action=helpdesk;sa=ajax;op=ajax_privacy;ticket=' . $context['ticket_id'] . '",
		sPostUrl: "action=helpdesk;sa=ajax;op=ajax_privacy2;ticket=' . $context['ticket_id'] . '",
	...
	if ($context['ticket']['urgency']['increase'] || $context['ticket']['urgency']['decrease'])
		add_js('
	var ajaxUrgency = new AjaxSelector({
		sSelf: "ajaxUrgency",
		sListItem: "item_urgency",
		sQueryUrl: "action=helpdesk;sa=ajax;op=ajax_urgency;ticket=' . $context['ticket_id'] . '",
		sPostUrl: "action=helpdesk;sa=ajax;op=ajax_urgency2;ticket=' . $context['ticket_id'] . '",
		sImagesUrl: "' . $context['plugins_url']['Arantor:WedgeDesk'] . '/images",
		sImageCollapsed: "ajax_assign.png",
		sImageExpanded: "ajax_assign_cancel.png"
	});');
	...
	function goAdvanced()
	{
		document.getElementById("shd_bbcbox").style.display = ' . (!empty($settings['shd_allow_ticket_bbc']) ? '""' : '"none"') . ';
		document.getElementById("shd_smileybox").style.display = ' . (!empty($settings['shd_allow_ticket_smileys']) ? '""' : '"none"') . ';
	...
	$context['display_ticket_log'] = !empty($settings['shd_display_ticket_logs']) && (shd_allowed_to('shd_view_ticket_logs_any', $context['ticket']['dept']) || (shd_allowed_to('shd_view_ticket_logs_own', $context['ticket']['dept']) && $context['ticket']['ticket_opener']));
	...
	if (wesql::num_rows($query) != 0)
		list($notify_state) = wesql::fetch_row($query);
	wesql::free_result($query);
	...
	if (!empty($context['display_notifications']['preferences']) || $context['display_notifications']['can_monitor'] || $context['display_notifications']['can_ignore'] || $context['display_notifications']['can_change'])
		wetem::load('viewnotifications', 'sidebar', 'add');
}
 ...
 *		<li>member: hash array containing details of the poster; normally the return value from Wedge's loadMemberContext() function. A minimal set of details is prepared if the poster holds no current Wedge account. Common values:
 *			<ul>
...
function shd_prepare_ticket_context()
{
	...
	$message = wesql::fetch_assoc($reply_request);
	if (!$message)
	{
		wesql::free_result($reply_request);
		return false;
	}
	...
	if (shd_allowed_to('shd_view_ip_any', $context['ticket']['dept']) || ($message['id_member'] == MID && shd_allowed_to('shd_view_ip_own', $context['ticket']['dept'])))
	{
		...
		if (!empty($ip))
			$output['ip_address'] = $context['link_ip_address'] ? ('<a href="<URL>?action=trackip;searchip=' . $ip . '">' . $ip . '</a>') : $ip;
		else
			$output['ip_address'] = '';
	}
...
function shd_display_load_attachments()
{
	...
	if ($settings['shd_attachments_mode'] == 'ticket')
	{
		$query = wesql::query('
			...
			FROM {db_prefix}helpdesk_attachments AS hda
				INNER JOIN {db_prefix}attachments AS a ON (hda.id_attach = a.id_attach)' . (empty($settings['attachmentShowImages']) || empty($settings['attachmentThumbnails']) ? '' : '
		...
		while ($row = wesql::fetch_assoc($query))
			$context['ticket_attach']['ticket'][$row['id_attach']] = shd_attachment_info($row);

		if (!empty($context['ticket_attach']['ticket']))
		{
			ksort($context['ticket_attach']['ticket']);
			wetem::load('viewticketattach', 'sidebar', 'add');
		}
		else
			$context['ticket_attach']['ticket'] = array();
	}
	else
	{
		if (empty($context['ticket_messages']))
		...
		while ($row = wesql::fetch_assoc($query))
			$context['ticket_attach'][$settings['shd_attachments_mode']][$row['id_msg']][$row['id_attach']] = shd_attachment_info($row);

		foreach ($context['ticket_attach'][$settings['shd_attachments_mode']] as $msg => $attachments)
		{
			if (empty($context['ticket_attach'][$settings['shd_attachments_mode']][$msg]))
				$context['ticket_attach'][$settings['shd_attachments_mode']][$msg] = array();

			ksort($context['ticket_attach'][$settings['shd_attachments_mode']][$msg]);
		}
	}
...
function shd_attachment_info($attach_info)
{
	...
	$attach = array(
		...
		'href' => '<URL>?action=dlattach;ticket=' . $context['ticket_id'] . '.0;attach=' . $attach_info['id_attach'],
		'link' => shd_attach_icon($filename) . '&nbsp;<a href="<URL>?action=dlattach;ticket=' . $context['ticket_id'] . '.0;attach=' . $attach_info['id_attach'] . '">' . htmlspecialchars($attach_info['filename']) . '</a>',
		'is_image' => !empty($settings['attachmentShowImages']) && !empty($attach_info['width']) && !empty($attach_info['height']),
	...
	if ($attach['is_image'])
	{
		...
		if (!empty($settings['attachmentThumbnails']) && !empty($settings['attachmentThumbWidth']) && !empty($settings['attachmentThumbHeight']) && ($attach_info['width'] > $settings['attachmentThumbWidth'] || $attach_info['height'] > $settings['attachmentThumbHeight']) && strlen($attach_info['filename']) < 249)
		{
			// A proper thumb doesn't exist yet? Create one!
			if (empty($attach_info['id_thumb']) || $attach_info['thumb_width'] > $settings['attachmentThumbWidth'] || $attach_info['thumb_height'] > $settings['attachmentThumbHeight'] || ($attach_info['thumb_width'] < $settings['attachmentThumbWidth'] && $attach_info['thumb_height'] < $settings['attachmentThumbHeight']))
			{
		...
		if (!empty($attach_info['id_thumb']))
			$attach['thumbnail'] = array(
				...
				'href' => '<URL>?action=dlattach;ticket=' . $context['ticket_id'] . '.0;attach=' . $attach_info['id_thumb'] . ';image',
			);
		$attach['thumbnail']['has_thumb'] = !empty($attach_info['id_thumb']);

		// If thumbnails are disabled, check the maximum size of the image.
		if (!$attach['thumbnail']['has_thumb'] && ((!empty($settings['max_image_width']) && $attach_info['width'] > $settings['max_image_width']) || (!empty($settings['max_image_height']) && $attach_info['height'] > $settings['max_image_height'])))
		{
			if (!empty($settings['max_image_width']) && (empty($settings['max_image_height']) || $attach_info['height'] * $settings['max_image_width'] / $attach_info['width'] <= $settings['max_image_height']))
			{
				$attach['width'] = $settings['max_image_width'];
				$attach['height'] = floor($attach_info['height'] * $settings['max_image_width'] / $attach_info['width']);
			}
			elseif (!empty($settings['max_image_width']))
			{
				$attach['width'] = floor($attach['width'] * $settings['max_image_height'] / $attach['height']);
				$attach['height'] = $settings['max_image_height'];
			}
		}
...
function shd_attach_icon($filename)
{
	...
	// If adding to this list, please remember to update WedgeDesk.english.php with a description in $txt['shd_attachtype_' . $extension]
	...
	if (isset($extension_map[$extension]))
		$image = '<img src="' . $context['plugins_url']['Arantor:WedgeDesk'] . '/images/attach/' . $extension_map[$extension] . '"' . (!empty($txt['shd_attachtype_' . $extension]) ? ' alt="' . $txt['shd_attachtype_' . $extension] . '" title="' . $txt['shd_attachtype_' . $extension] . '"' : '') . '>';
	else
		$image = '<img src="' . $context['plugins_url']['Arantor:WedgeDesk'] . '/images/attach/blank.png">';
 ...
 *	@since 2.0
*/
function shd_load_relationships()
{
	global $context, $txt, $settings;

	$context['create_relationships'] = shd_allowed_to('shd_create_relationships', $context['ticket']['dept']);
	$context['display_relationships'] = (shd_allowed_to('shd_view_relationships', $context['ticket']['dept']) || $context['create_relationships']) && empty($settings['shd_disable_relationships']);
	...
	if (!empty($context['relationships_count']) || !empty($context['create_relationships']))
		wetem::load('viewrelationships', 'sidebar', 'add');
}
