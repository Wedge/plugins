<?php
/**
 * Wedge: Birthdays
 *
 * Language strings for the administration panel.
 *
 * @package wedge-birthdays
 * @copyright 2010-2011 Wedge Team, wedge.org, 2011 Simple Machines, simplemachines.org
 * @license http://wedge.org/license/
 */

$txt['birthdays'] = 'Birthdays';
$txt['birthday_info_center'] = 'Show upcoming birthdays in the info center?';
$txt['birthday_info_center_days'] = 'For how many days?';
$txt['scheduled_task_birthdays'] = 'Birthday Processing';
$txt['scheduled_task_desc_birthdays'] = 'Daily processing needed for birthdays, including sending out notifications.';

$txt['birthday_show_ages'] = 'Show the ages of forum members';

$txt['birthday_send_email'] = 'Send emails out to users on their birthday?';
$txt['birthday_email'] = 'Birthday Message to use';
$txt['birthday_body'] = 'Email Body';
$txt['birthday_subject'] = 'Email Subject';

$txt['help_birthday_email'] = 'Choose the index of the birthday email message to use. A preview will be shown in the Email Subject and Email Body fields.<br><strong>Note:</strong> This requires that the Birthdays scheduled task is enabled, which it should be after installing this plugin. You can check the <a href="<URL>?action=admin;area=scheduledtasks" target="_blank" class="new_win">Scheduled Tasks</a> page if you wish.';

?>