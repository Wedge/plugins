<?php
/**
 * Wedge: Birthdays
 *
 * Templates of the emails to send out. Formatted as they are to allow editing from the language editor.
 *
 * happy_birthday was written by Thantos (http://www.simplemachines.org/community/?action=profile;u=2676)
 * karlbenson1 and karlbenson2 were written by Karl Benson (http://www.simplemachines.org/community/?action=profile;u=63186)
 * nite0859 was written by nite0859 (http://www.simplemachines.org/community/?action=profile;u=46625)
 * zwaldowski was written by zwaldowski (http://www.simplemachines.org/community/?action=profile;u=72038)
 * geezmo was written by geezmo (http://www.simplemachines.org/community/?action=profile;u=48671)
 *
 * @package wedge-birthdays
 * @copyright 2010-2011 Wedge Team, wedge.org, 2011 Simple Machines, simplemachines.org
 * @license http://wedge.org/license/
 */

$txt['birthday_template_subject_happy_birthday'] = 'Joyeux anniversaire de la part de {FORUMNAME}.';
$txt['birthday_template_body_happy_birthday'] = 'Cher {REALNAME},' . "\n\n" . '{FORUMNAME} vous souhaite un joyeux anniversaire. Que ce jour et l\'ann&#233;e &#224; suivre puissent vous remplir de joie.' . "\n\n" . '{REGARDS}';

$txt['birthday_template_subject_karlbenson1'] = 'Pour votre anniversaire...';
$txt['birthday_template_body_karlbenson1'] = 'Nous aurions pu vous envoyer une carte d\'anniversaire. Nous aurions pu vous envoyer des fleurs ou un g&#226;teau.' . "\n\n" . 'Mais nous ne l\'avons pas fait.' . "\n\n" . 'Nous aurions m&#234;me pu vous envoyer un de ces messages g&#233;n&#233;r&#233;s automatiquement pour vous souhaiter un joyeux anniversaire o&#367; nous n\'avions m&#234;me pas &#224; remplacer le NOM &#192; INS&#201;RER.' . "\n\n" . 'Mais nous ne l\'avons pas fait.' . "\n\n" . 'Nous avons &#233;crit ces voeux rien que pour vous.' . "\n\n" . 'Nous tenons &#224; vous souhaiter un anniversaire plein de bonheur.' . "\n\n" . '{REGARDS}' . "\n\n" . '//:: Ce message a &#233;t&#233; g&#233;n&#233;r&#233; automatiquement :://';

$txt['birthday_template_subject_nite0859'] = 'Joyeux anniversaire !';
$txt['birthday_template_body_nite0859'] = 'Vos amis sur {FORUMNAME} voudraient prendre un peu de votre temps pour vous souhaiter un joyeux anniversaire, {REALNAME}. Si vous ne l\'avez pas fait r&#233;cemment, pensez &#224; visiter notre communaut&#233; afin que d\'autres aient l\'opportunit&#233; de vous transmettre leurs voeux !' . "\n\n" . 'M&#234;me si aujourd\'hui c\'est votre anniversaire, {REALNAME}, nous tenons &#224; vous rappeler que votre appartenance &#224; notre communaut&#233; a &#233;t&#233; le plus beau cadeau pour nous &#224; ce jour.' . "\n\n" . 'Meilleurs voeux,' . "\n" . 'L\'&#233;quipe de {FORUMNAME}';

$txt['birthday_template_subject_zwaldowski'] = 'Voeux d\'anniversaire &#224; {REALNAME}';
$txt['birthday_template_body_zwaldowski'] = 'Cher {REALNAME},' . "\n\n" . 'Une autre ann&#233;e de votre vie s\'est &#233;coul&#233;e. {FORUMNAME} esp&#232;re qu\'elle a &#233;t&#233; remplie de bonheur, et vous souhaite bonne chance pour la prochaine.' . "\n\n" . '{REGARDS}';

$txt['birthday_template_subject_geezmo'] = 'Joyeux anniversaire, {REALNAME} !';
$txt['birthday_template_body_geezmo'] = 'Savez-vous qui f&#234;te son anniversaire aujourd\'hui, {REALNAME} ?' . "\n\n" . 'Nous oui... VOUS !' . "\n\n" . 'Joyeux anniversaire !' . "\n\n" . 'Vous avez maintenant un an de plus mais nous esp&#233;rons que vous &#234;tes beaucoup plus heureux que l\'ann&#233;e derni&#232;re.' . "\n\n" . 'Profitez de votre journ&#233;e d\'aujourd\'hui, {REALNAME} !' . "\n\n" . '- De la part de votre famille {FORUMNAME}';

$txt['birthday_template_subject_karlbenson2'] = 'Vos voeux d\'anniversaire';
$txt['birthday_template_body_karlbenson2'] = 'Nous esp&#233;rons que votre anniversaire est le meilleur que vous ayez eu quel que soit le temps, nuageux ou ensoleill&#233;.' . "\n" . 'Ayez beaucoup de g&#226;teaux d\'anniversaire et de plaisir, et vous nous raconterez comment &#231;a s\'est pass&#233;.' . "\n\n" . 'Nous esp&#233;rons que ce message vous a fait plaisir et que cela continuera. &#192; l\'ann&#233;e prochaine, m&#234;me endroit m&#234;me heure !' . "\n\n" . '{REGARDS}';

?>